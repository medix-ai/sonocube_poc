...
def build_pdf(report_path: Path, analysis_result: Dict[str, Any]):
    ...
    # 구조지표 섹션
    structure_metrics = analysis_result.get("structure_metrics", {})
    if structure_metrics:
        story.append(Paragraph("<b>Structure Metrics</b>", styles['Heading2']))
        story.append(Spacer(1, 0.3*cm))
        ...
        structure_data = [
            ["Metric", "Value"],
            ["Left Atrium Volume (LA)", f"{la_volume:.1f} ml"],
            ["Right Atrium Volume (RA)", f"{ra_volume:.1f} ml"],
        ]
        
        # Wall Thickness 상세 정보
        if wall_thickness:
            wt_septal = wall_thickness.get("septal", 0.0)
            wt_lateral = wall_thickness.get("lateral", 0.0)
            wt_anterior = wall_thickness.get("anterior", 0.0)
            wt_inferior = wall_thickness.get("inferior", 0.0)
            wt_avg = wall_thickness.get("average", 0.0)
            
            structure_data.append(["Wall Thickness (Average)", f"{wt_avg:.1f} mm"])
            structure_data.append(["  - Septal", f"{wt_septal:.1f} mm"])
            structure_data.append(["  - Lateral", f"{wt_lateral:.1f} mm"])
            structure_data.append(["  - Anterior", f"{wt_anterior:.1f} mm"])
            structure_data.append(["  - Inferior", f"{wt_inferior:.1f} mm"])
        
        structure_data.append(["Sphericity Index", f"{sphericity_index:.2f}"])
        
        structure_table = Table(structure_data, colWidths=[6*cm, 4*cm])
        structure_table.setStyle(TableStyle([
            ...
            ('BACKGROUND', (0, 1), (-1, -1), colors.beige),
            ('GRID', (0, 0), (-1, -1), 1, colors.black),
            ('FONTSIZE', (0, 3), (0, 6), 10),  # Wall thickness 세부 항목 작은 폰트
            ('LEFTPADDING', (0, 3), (0, 6), 20),  # 들여쓰기
        ]))
